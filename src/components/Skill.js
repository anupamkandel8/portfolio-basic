            ...
            <span className="flex flex-wrap gap-3 mt-0">
              {category.items.map((skill, i) => (
                ...
                >
                  ...
                  {/* Tooltip */}
                  <span
                    className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 px-2 py-1 rounded bg-gray-800 text-white text-xs opacity-0 group-hover:opacity-100 group-focus:opacity-100 pointer-events-none transition-opacity z-10 whitespace-nowrap"
                  >
                    {skill.name}
                  </span>
                </span>
